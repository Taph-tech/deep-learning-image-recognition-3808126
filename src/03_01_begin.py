...
def display_images(images, labels, y_data, rows=4, cols=4, save_path=None):
    ...
    if save_path:
        plt.savefig(f'Plot saved to {save_path}')
...
# Display a sample of training images with labels and save the plot
...
# Illustrate the simple CNN architecture
illustrate_simple_cnn()

# Define a function to explain convulution operation
def explain_covultion():
  # Create a 3X3 image with a single channel
...
def explain_pooling():
    # Create a 4X4 image with a single channel
